...
$.widget( "evol.colorpicker", {
	...
	options: {
		...
		strings: '主题颜色,标准颜色,更多颜色,返回,回到调色板,使用过的,暂无记录.'
	},
            ...
            default:
				...
				e.addClass('colorPicker '+this._id)
					...
					.wrap('<div style="width:'+(this.options.hideButton?this.element.width():this.element.width()+32)+'px;'+
						...
						'"></div>')
                        */
					//.after('<div class="'+css+' ng-hide" style="'+style+'"></div>')
	...
	_bindColors: function() {
		...
		this._palette
			...
			.on('mouseenter', sel, function(evt){
                clearTimeout(h);
				...
				if(that._enabled){
					...
					if(that._active){
                        ...
                        h = setTimeout(function(){
                            that.options.onChange && that.options.onChange(c, c.replace(/^#/g, ''));
                        }, 500);
					}
				}
			})
			.on('mouseleave', sel, function(evt){
                    var c = that.val();
					if(that._active){
                        //that.options.onChange && that.options.onChange(c, c.replace(/^#/g, ''));
                        clearTimeout(h);
	...
	_setValue: function(c, noHide) {
        ...
        this.options.onChange && this.options.onChange(c, c.replace(/^#/g, ''));
	},
